      ...
      >
        ...
        >
          ...
          <div class="flex items-start justify-between gap-3 px-5 sm:px-6 py-4 border-b border-slate-100 shrink-0">
            <div class="min-w-0">
              <h3 class="text-base font-bold text-slate-900">Cetak Kartu Peserta</h3>
          ...
          <div class="px-5 sm:px-6 py-3 border-t border-slate-100 flex flex-wrap items-center justify-between gap-3 shrink-0">
            <p class="text-[11px] text-slate-500">
              Kertas <strong>A4</strong>, margin <strong>Default/None</strong>, skala 100%. Siswa login dengan No. Ujian &amp; password selama event aktif.
            </p>
...
const cardHtml = (p) => `
  <div class="card${String(p.full_name || '').length > LONG_NAME ? ' long' : ''}">
    ...
    <div class="inner">
      ...
      <table class="rows">
        ...
        <tr><td class="k">Password</td><td class="v">: ${escapeHtml(p.password)}</td></tr>
      </table>
      <div class="foot">
...
const documentHtml = computed(() => {
  ...
  return `<!DOCTYPE html>
<html lang="id"><head><meta charset="utf-8"><title>Kartu Peserta - ${escapeHtml(props.event?.title || '')}</title>
<style>
